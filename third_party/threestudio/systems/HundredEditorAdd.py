...
from editor.gaussiansplatting.scene.vanilla_gaussian_model import (
    GaussianModel as VanillaGaussianModel,
)
from trainer.origin.utils.graphics_utils import fov2focal
...
WONDER3D_ROOT = REPO_ROOT / "third_party" / "threestudio" / "utils" / "wonder3D"
TRAIN_FROM_MESH_CANDIDATES = [
    REPO_ROOT / "train_from_mesh.py",
    REPO_ROOT / "src" / "train_from_mesh.py",
]


def resolve_train_from_mesh_script() -> Path:
    for candidate in TRAIN_FROM_MESH_CANDIDATES:
        if candidate.exists():
            return candidate
    raise FileNotFoundError(
        "Cannot find train_from_mesh.py in expected locations: "
        + ", ".join(str(p) for p in TRAIN_FROM_MESH_CANDIDATES)
    )
...
class HundredEditorAdd(HundredEditor):
    ...
    def on_fit_start(self) -> None:
        ...
        if self.cfg.cache_overwrite or not os.path.exists(mesh_path):
            ...
            p2 = subprocess.Popen(
                cmd,
                cwd=str(WONDER3D_ROOT / "instant-nsr-pl"),
            )
            p2.wait()

        if self.cfg.cache_overwrite or not os.path.exists(gs_path):
            train_from_mesh_script = resolve_train_from_mesh_script()
            p3 = subprocess.Popen(
                [
                    f"{sys.prefix}/bin/python",
                    str(train_from_mesh_script),
                    "--mesh",
                    mesh_path,
                    "--save_path",
                    gs_path,
