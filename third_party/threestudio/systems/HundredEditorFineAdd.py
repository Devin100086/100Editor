...
@threestudio.register("gsedit-system-fine-add")
class HundredEditorFineAdd(HundredEditor):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        if len(self.cfg.cache_dir) > 0:
            self.cache_dir = os.path.join("fine_add_cache", self.cfg.cache_dir)
        else:
            self.cache_dir = os.path.join("fine_add_cache", self.cfg.gs_source.replace("/", "-"))
