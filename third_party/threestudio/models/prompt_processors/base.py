...
class PromptProcessor(BaseObject):
    ...
    def configure(self) -> None:
        ...
        prompt_library_candidates = [
            Path.cwd() / "load" / "prompt_library.json",
            Path(__file__).resolve().parents[2] / "load" / "prompt_library.json",
        ]
        prompt_library_path = next(
            (p for p in prompt_library_candidates if p.is_file()), None
        )
        if prompt_library_path is None:
            threestudio.warn(
                "prompt_library.json not found. Falling back to an empty prompt "
                "library. Add file at "
                f"{(Path(__file__).resolve().parents[2] / 'load' / 'prompt_library.json').as_posix()} "
                "if you use prompts starting with 'lib:'."
            )
            self.prompt_library = {"dreamfusion": []}
        else:
