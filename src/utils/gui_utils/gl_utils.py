...
def draw_click_ripples(max_w, max_h, ripples):
    if ripples is None or len(ripples) == 0:
        return

    gl.glPushAttrib(gl.GL_ENABLE_BIT | gl.GL_LINE_BIT | gl.GL_COLOR_BUFFER_BIT)
    ...
    for ripple in ripples:
        x = float(ripple.get("x", 0))
        y = float(ripple.get("y", 0))
        radius = max(1.0, float(ripple.get("radius", 1.0)))
        alpha = float(np.clip(ripple.get("alpha", 0.0), 0.0, 1.0))
        line_width = max(1.0, float(ripple.get("line_width", 1.0)))
