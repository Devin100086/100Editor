...
class FittingRenderer(Renderer):
    def __init__(self,host,port):
        super().__init__()
        self.connector = AsyncConnector(0.05, host, port)
        self.host = host
        self.port = port
    ...
    def restart_connector(self):
        self.connector = AsyncConnector(0.05, self.host, self.port)
    ...
    def _parse_resolution(resolution) -> Tuple[int, int]:
        ...
        raise ValueError(f"Unsupported img_size: {resolution}")
    ...
    def read(self, resolution):
        try:
            ...
            if isinstance(resolution, (tuple, list)) and len(resolution) == 2:
                return torch.zeros([3, int(resolution[0]), int(resolution[1])]), {}
    ...
    def _render_impl(
        ...
        **other_args,
        ...
        for current_image in images:
            if current_image.shape[-2:] != (target_h, target_w):
