...
class AttachRenderer(Renderer):
    def __init__(self, host, port):
        ...
        self.host = host
        self.port = port
        # Keep clipping planes aligned with 3DGS camera defaults to avoid
        # aggressive far-plane clipping when users move the camera away.
        self.z_near = 0.01
        self.z_far = 100.0
    ...
    def _render_impl(
        ...
        **other_args,
        ...
        message = {
            ...
            "slider": slider,
            "render_alpha": bool(render_alpha),
            "render_depth": bool(render_depth),
        ...
        if len(stats.keys()) > 0:
            ...
            if "mean_xyz" in stats:
                res.mean_xyz = torch.tensor(stats["mean_xyz"], dtype=torch.float32)
            if "std_xyz" in stats:
                res.std_xyz = torch.tensor(stats["std_xyz"], dtype=torch.float32)
        self._return_image(
            image,
            res,
