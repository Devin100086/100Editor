...
from editor.gaussiansplatting.arguments import (
    PipelineParams,
    OptimizationParams,
)
...
from src.utils.path_utils import resolve_runtime_subdir, resolve_sam2_paths
...
class BaseTrainer:
    ...
    def _append_output_log(self, filename: str, line: str) -> None:
        log_path = self._resolve_output_log_path(filename)
        with open(log_path, "a") as f:
            f.write(line)

    def _get_mask_save_dir(self) -> Path:
        if getattr(self, "save_mask_tmp", None):
            mask_dir = Path(self.save_mask_tmp)
        else:
            mask_dir = resolve_runtime_subdir(
                __file__, "cache", "edit", "mask", create=True
            )
            self.save_mask_tmp = mask_dir.as_posix()
    ...
    def update_mask(self, edit_cameras, text_prompt = "hat", type = "default") -> None:
        ...
        mask_dir = self._get_mask_save_dir()
        ...
        with torch.inference_mode():
            for cam in tqdm(
                ...
                torchvision.utils.save_image(
                    mask_float.unsqueeze(0).to(torch.float16),
                    (mask_dir / f"mask_{cam.image_name}.png").as_posix(),
                )
                masks.append((mask.detach().to("cpu").numpy().astype(np.uint8) * 255))
                self.gaussian.apply_weights(cur_cam, weights, weights_cnt, mask_float)
                del this_frame, mask, mask_float
    ...
    def update_sam_mask_with_point_prompt(
        ...
        sam2_model_cfg, sam2_checkpoint = resolve_sam2_paths(__file__)
    ...
    def compute_clip(self, step, clip_prompt_origin = "a photo of a face of a man", clip_prompt_target = "a photo of a face of vampire"):
        ...
        avg_cos = total_cos / len(self.colmap_cameras)
        avg_sim = total_sim / len(self.colmap_cameras)
        self._append_output_log(
            "clip_metric_log.txt",
            f"origin: {clip_prompt_origin} | target: {clip_prompt_target} | cos: {avg_cos} | sim: {avg_sim}\n",
        )
        with open(self._resolve_output_log_path("Ours(CLIP_DIRECTION).txt"), "a") as f:
            if step == 0:
                f.write(str(avg_cos))
            else:
                f.write("," + str(avg_cos))
        with open(self._resolve_output_log_path("Ours(CLIP).txt"), "a") as f:
            if step == 0:
                f.write(str(avg_cos))
            else:
                f.write("," + str(avg_sim))
        return avg_sim, avg_cos
    ...
    def compute_metric(self, clip_prompt_origin = "a photo of a face of a man", clip_prompt_target = "a photo of a face of Harry Potter"):
        ...
        with torch.no_grad():
            for id, cam in enumerate(self.test_cameras):
                ...
                total_cos += abs(cos_sim.item())
                total_sim += abs(sim.item())
        avg_cos = total_cos / len(self.test_cameras)
        avg_sim = total_sim / len(self.test_cameras)
        self._append_output_log(
            "earlystop_metric_log.txt",
            f"origin: {clip_prompt_origin} | target: {clip_prompt_target} | cos: {avg_cos} | sim: {avg_sim}\n",
        )
        return avg_sim, avg_cos
