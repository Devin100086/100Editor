...
class HundredEditorNetwork:
    ...
    def render(self, pipe, gaussians, loss, render, background, iteration, opt, hold=False, render_alpha_fn=None):
        if self.conn == None:
            self.try_connect()
        while self.conn != None:
            ...
            try:
                ...
                if self.custom_cam != None:
                    render_background = self.bg_color if self.bg_color is not None else background
                    with torch.no_grad():
                        if self.render_alpha and render_alpha_fn is not None:
                            alpha_pkg = render_alpha_fn(self.custom_cam, gs, pipe, render_background, self.scaling_modifer)
                            net_image = alpha_pkg["render"]
                        else:
                            render_pkg = render(self.custom_cam, gs, pipe, render_background, self.scaling_modifer)
                            if self.render_depth:
                                net_image = self._depth_to_rgb(render_pkg["depth"])
                            else:
                ...
                training_stats = json.dumps({
                    ...
                    "mean_xyz": gaussians.get_xyz.detach().mean(dim=0).cpu().tolist(),
                    "std_xyz": gaussians.get_xyz.detach().std(dim=0).cpu().tolist(),
