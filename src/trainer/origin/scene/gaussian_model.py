...
import torch
import numpy as np
from trainer.origin.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation
from torch import nn
import os
import json
from trainer.origin.utils.system_utils import mkdir_p
from plyfile import PlyData, PlyElement
from trainer.origin.scene.appearance import AppearanceNetwork
from trainer.origin.utils.sh_utils import RGB2SH
from simple_knn._C import distCUDA2
from trainer.origin.utils.graphics_utils import BasicPointCloud
from trainer.origin.utils.general_utils import strip_symmetric, build_scaling_rotation
...
class GaussianModel:
    ...
    def abs_densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        n_init_points = self.get_xyz.shape[0]
        # Extract points that satisfy the gradient condition
        padded_grad = torch.zeros((n_init_points), device="cuda")
        padded_grad[:grads.shape[0]] = grads.squeeze()
    ...
    def abs_densify_and_prune(self, max_grad, max_grad_abs, min_opacity, extent, max_screen_size, radii):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0
        ...
        self.tmp_radii = radii
        self.densify_and_clone(grads, max_grad, extent)
        self.abs_densify_and_split(grads_abs, max_grad_abs, extent)
    ...
    def add_abs_densification_stats(self, viewspace_point_tensor, update_filter):
        self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor.grad[update_filter,:2], dim=-1, keepdim=True)
