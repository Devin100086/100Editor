...
import traceback
import socket
from trainer.gsplat.render import rasterize_splats
import json
import math
...
class GsplatNetwork:
    ...
    def receive(self):
        ...
        if width != 0 and height != 0:
            try:
                ...
                self.render_alpha = bool(message.get("render_alpha", False))
                self.render_depth = bool(message.get("render_depth", False))
                bg_color = message.get("background_color", [0.0, 0.0, 0.0])
    ...
    def _depth_to_rgb(depth):
        ...
        return depth.expand(*depth.shape[:-1], 3)

    def render(self, sh_degree, gsplat, loss, world_size, iteration, device, opt, hold=False):
        if self.conn == None:
            self.try_connect()
        while self.conn != None:
            ...
            try:
                ...
                if self.custom_cam != None:
                    with torch.no_grad():
                        use_depth_channel = bool(opt.depth_loss) or self.render_depth
                        renders_network, alpha_channel, _ = rasterize_splats(
                            splats=gsplat,
                            cfg=opt,
                            ...
                            render_mode="RGB+ED" if use_depth_channel else "RGB",
                        )
                        # Always fetch alpha for visualization compositing / alpha view.
                        if use_depth_channel:
                            color_channels = renders_network[..., 0:3]
                            depth_channel = renders_network[..., 3:4]
                        else:
                            color_channels = renders_network
                            depth_channel = None
                        if self.render_alpha:
                            alpha_vis = alpha_channel.expand(*alpha_channel.shape[:-1], 3)
                            net_image = alpha_vis.reshape(-1, *alpha_vis.shape[2:])
                        elif self.render_depth:
                            if depth_channel is None:
                                depth_vis = torch.zeros_like(color_channels)
                            else:
                                depth_vis = self._depth_to_rgb(depth_channel)
                            net_image = depth_vis.reshape(-1, *depth_vis.shape[2:])
                        else:
                            bg = self.bg_color.to(color_channels.dtype).view(1, 1, 1, 3)
                            colors = color_channels + bg * (1.0 - alpha_channel)
                ...
                training_stats = json.dumps({
                    ...
                    "mean_xyz": gsplat["means"].detach().mean(dim=0).cpu().tolist(),
                    "std_xyz": gsplat["means"].detach().std(dim=0).cpu().tolist(),
                    "sh_degree": sh_degree,
                ...
                if hold:
                    if (not self.do_training) or self.single_training_step:
                        break
                    continue
