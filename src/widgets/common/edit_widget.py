...
from utils.gui_utils.easy_json import load_json, save_json
...
from third_party.gaussiansplatting.scene.gaussian_model import GaussianModel
...
class EditWidget(Widget):
    def __init__(self, viz):
        ...
        gui_config_dir = repo_root / "configs" / "gui_configs"
        self.preset_path = str(gui_config_dir / "presets.json")
        self.history_path = str(gui_config_dir / "history.json")
        self.load_presets()
    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            with imgui_utils.change_font(self.viz._imgui_fonts_code[self.viz._cur_font_size + increase_font_size]):
                ...
                editor_text_active = bool(
                    imgui.is_item_active()
                    or imgui.is_item_focused()
                    or getattr(imgui.get_io(), "want_text_input", False)
                )
        ...
        viz._editor_text_active = editor_text_active
